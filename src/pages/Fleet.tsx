...
import { Users, MapPin, Clock, ArrowRight, ChevronLeft, ChevronRight } from 'lucide-react';
...
const FleetCard = ({ category, index }: { category: AircraftCategory; index: number }) => {
  ...
  return (
    ...
    >
      <div className="relative h-full p-8 md:p-10 rounded-2xl border border-white/[0.06] bg-white/[0.02] hover:bg-white/[0.04] hover:border-[hsl(var(--champagne))/0.2] transition-all duration-500">
        {/* Category Number */}
        <span className="absolute top-6 right-6 text-5xl font-serif font-light text-white/[0.03] group-hover:text-[hsl(var(--champagne))/0.08] transition-colors duration-500">
          0{index + 1}
        </span>

        {/* Header */}
        <div className="mb-8">
          <span className="text-[10px] font-medium tracking-[0.25em] uppercase text-[hsl(var(--champagne))]">
            {category.tagline}
          </span>
          <h3 className="text-2xl md:text-3xl font-serif font-light text-[hsl(var(--cream))] mt-2">
            {category.category}
          </h3>
        </div>

        {/* Description */}
        <p className="text-[hsl(var(--cream-muted))] leading-relaxed mb-8">
          {category.description}
        </p>

        {/* Specs Grid */}
        <div className="grid grid-cols-3 gap-3 mb-8">
          <div className="p-4 rounded-xl bg-white/[0.03] border border-white/[0.04] text-center group-hover:border-white/[0.08] transition-colors duration-300">
            <Users className="w-4 h-4 text-[hsl(var(--champagne))] mx-auto mb-2" />
            <div className="text-lg font-serif text-[hsl(var(--cream))]">{category.passengers}</div>
            <div className="text-[10px] text-[hsl(var(--cream-muted))] uppercase tracking-wider mt-1">Passengers</div>
          </div>
          <div className="p-4 rounded-xl bg-white/[0.03] border border-white/[0.04] text-center group-hover:border-white/[0.08] transition-colors duration-300">
            <MapPin className="w-4 h-4 text-[hsl(var(--champagne))] mx-auto mb-2" />
            <div className="text-lg font-serif text-[hsl(var(--cream))]">{category.range.split(' ')[0]}</div>
            <div className="text-[10px] text-[hsl(var(--cream-muted))] uppercase tracking-wider mt-1">Range</div>
          </div>
          <div className="p-4 rounded-xl bg-white/[0.03] border border-white/[0.04] text-center group-hover:border-white/[0.08] transition-colors duration-300">
            <Clock className="w-4 h-4 text-[hsl(var(--champagne))] mx-auto mb-2" />
            <div className="text-lg font-serif text-[hsl(var(--cream))]">{category.flightTime.split(' ')[0]}</div>
            <div className="text-[10px] text-[hsl(var(--cream-muted))] uppercase tracking-wider mt-1">Flight Time</div>
          </div>
        </div>

        {/* Models */}
        <div className="border-t border-white/[0.06] pt-6">
          <span className="text-[10px] font-medium tracking-[0.2em] uppercase text-[hsl(var(--cream-muted))] block mb-4">
            Sample Aircraft
          </span>
          <ul className="space-y-2">
            {category.models.slice(0, 3).map((model) => (
              <li key={model} className="flex items-center gap-3 text-sm text-[hsl(var(--cream-muted))]">
                <span className="w-1.5 h-1.5 rounded-full bg-[hsl(var(--champagne))/0.5]" />
                {model}
              </li>
            ))}
            {category.models.length > 3 && (
              <li className="text-xs text-[hsl(var(--cream-muted))]/50 pl-5">
        ...
        <div className="mt-6 pt-6 border-t border-white/[0.04]">
          <span className="text-xs text-[hsl(var(--cream-muted))]">
            <span className="text-[hsl(var(--champagne))]">Ideal for:</span> {category.idealFor}
          </span>
        </div>
      </div>
    </div>
...
const JetExplorer = () => {
  const [currentIndex, setCurrentIndex] = useState(2); // Start at midsize
    ...
    <section className="py-20 md:py-28 bg-[#f5f5f5] overflow-hidden">
      <div className="max-w-[1400px] mx-auto px-6 md:px-12">
        ...
        <div className="max-w-2xl mx-auto">
          ...
          {/* Explore Button */}
          <div className="text-center">
            <button className="px-12 py-4 border border-neutral-900 text-neutral-900 text-sm font-medium tracking-[0.2em] uppercase hover:bg-neutral-900 hover:text-white transition-all duration-300">
              Request Quote
            </button>
          </div>
        </div>
    ...
    <div className="min-h-screen scrollbar-luxury" style={{ backgroundColor: 'hsl(var(--charcoal-dark))' }}>
      ...
      <section ref={heroRef} className="relative pt-32 pb-20 md:pt-40 md:pb-28 overflow-hidden">
        ...
        <div
          ...
          style={{
            background: 'radial-gradient(ellipse 80% 50% at 50% 0%, hsl(var(--champagne) / 0.08), transparent 60%)',
          }}
        />
        ...
        <div className="relative z-10 max-w-[1400px] mx-auto px-6 md:px-12">
          <div className="max-w-3xl">
            ...
            >
              <span className="w-12 h-px bg-[hsl(var(--champagne))]" />
              <span className="eyebrow">Our Network</span>
            </div>

            {/* Title */}
            <h1
              className={`font-serif font-light text-[hsl(var(--cream))] transition-all duration-1000 uppercase ${
                heroVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
              }`}
              style={{ transitionDelay: '100ms' }}
            >
              Fleet <span className="text-gradient-gold">Overview</span>
            </h1>

            {/* Subtitle */}
            <p
              className={`mt-6 body-large max-w-2xl transition-all duration-1000 ${
                heroVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
              }`}
              style={{ transitionDelay: '200ms' }}
            >
              Access to over 1,500 premium aircraft through our vetted operator network.
              We'll match you with the perfect aircraft for your mission.
            </p>
          </div>
        </div>
      </section>

      {/* Jet Explorer Section */}
      <JetExplorer />
        ...
        <div className="max-w-[1400px] mx-auto px-6 md:px-12">
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
            {fleetCategories.map((category, index) => (
              <FleetCard key={category.category} category={category} index={index} />
            ))}
          </div>
        </div>
      </section>
      ...
      <section className="relative py-24 md:py-32 border-t border-white/[0.04]">
        ...
        <div
          ...
          style={{
            background: 'radial-gradient(ellipse 60% 60% at 50% 100%, hsl(var(--champagne) / 0.06), transparent 60%)',
          }}
        />

        <div className="relative z-10 max-w-3xl mx-auto px-6 md:px-12 text-center">
          <span className="eyebrow block mb-6">Not Sure Which Category?</span>
          <h2 className="font-serif font-light text-[hsl(var(--cream))] uppercase">
            Tell us about your trip, and we'll recommend the{' '}
            <span className="italic text-[hsl(var(--champagne))]">perfect aircraft.</span>
          </h2>
          <p className="mt-6 body-large max-w-xl mx-auto">
            Our aviation experts analyze your route, schedule, and preferences to find the optimal solution.
          </p>
          <a
            href="/"
            className="group inline-flex items-center gap-2 mt-10 btn-luxury bg-[hsl(var(--champagne))] text-[hsl(var(--charcoal-dark))] rounded-full font-medium hover:bg-[hsl(var(--champagne-light))]"
          >
